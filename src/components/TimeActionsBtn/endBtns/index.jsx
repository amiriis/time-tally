    ...
    return (
        <Stack alignItems={"center"} spacing={0.5}>
            <ButtonGroup color="error" size="small" variant="outlined">
                <Button onClick={() => endNowHandler(work.id)} endIcon={<StopCircleIcon />}>
                    end now
                </Button>
                <EndToBtn work={work} />
            </ButtonGroup>
            <TimeInterval workId={work.id} start_at={moment(work.time_tracking_started_at.toDate())} hasWorkingHours={true} />
        </Stack>
    );
}
