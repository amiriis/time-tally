...
const TimesCalculator = ({ times, workingHours }) => {
    ...
    const totalDuration = useMemo(() =>
        times.reduce((sum, time) => sum + time.total_time.duration, 0),
        [times]
    );
    ...
    return (
        <Stack alignItems='center' sx={{ p: 0.5 }}>
            <Stack direction='row' spacing={1} alignItems='center'>
                ...
                <Typography textAlign="center" fontWeight='bold' color={timeDifference.isGreater ? "success.main" : 'error.main'}>
                    {timeDifference.isGreater ? '+' : '-'}{String(timeDifference.duration.hours).padStart(2, "0")}:{String(timeDifference.duration.minutes).padStart(2, "0")}
                </Typography>
            </Stack>
        </Stack>
    );
